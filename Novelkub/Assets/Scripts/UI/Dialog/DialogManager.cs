...
public class DialogManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _MiniGame = new int[5, 3] { { 2200, 0, 5}, { (int)ObjectNum.NPC3 + (int)QuestNum.Third + 1, 0, 5 }, { (int)ObjectNum.NPC4 + (int)QuestNum.Forth, 0, 8 }, { (int)ObjectNum.NPC5 + (int)QuestNum.Fifth, 0, 7 }, { (int)ObjectNum.NPC5 + (int)QuestNum.Fifth + 1, 0, 10 } };
    }
    ...
    private void GenerateQuestDialogData()
    {
		...
		_dialogData.Add((int)ObjectNum.NPC7 + (int)QuestNum.Seventh,
						new string[] { "(어딘가 급해보이는 부인이 호텔을 나서고 있다):0",
									   ...
									   "(일련의 설전이 오가고, 부인의 캐리어에 배게처럼 보이는 물건이 \n살짝 빠져나와 있는 것이 보인다:0",
									   "(그리고 플레이어는 부인의 캐리어에서 배게를 확보하고 여자를 용의자로 체포한다:0"});

		_dialogData.Add((int)ObjectNum.NPC7 + (int)QuestNum.Seventh + 1,
						new string[] { "증거라면 부인께서 가지고 계시지않습니까?:0",
									   "무슨...!:1",
									   "현장에서 범행도구가 사라져있었죠.. \n하지만 사라진건 범행도구 뿐만이 아니었습니다.:0",
